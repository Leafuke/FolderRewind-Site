import type {SidebarsConfig} from '@docusaurus/plugin-content-docs';

const sidebars: SidebarsConfig = {
  docs: [
    'intro',
    {
      type: 'category',
      label: '开始使用',
      collapsed: false,
      items: [
        'getting-started/installation',
        'getting-started/v1-8-upgrade',
        'getting-started/first-backup',
        'getting-started/first-restore',
      ],
    },
    {
      type: 'category',
      label: '使用指南',
      items: [
        'guides/backup-modes',
        'guides/backup-file-spec',
        'guides/folder-management',
        'guides/templates',
        'guides/template-sharing',
        'guides/automation',
        'guides/cloud-archive',
        'guides/encryption',
        'guides/filters',
        'guides/history-timeline',
        'guides/mini-window',
        'guides/data-migration',
        {
          type: 'category',
          label: 'Minecraft 专题',
          items: [
            'guides/minecraft/overview',
            'guides/minecraft/minebackup-mod',
            {
              type: 'category',
              label: '联动组件 / Integration Components',
              items: [
                'guides/minecraft/minebackup-plugin',
                'guides/minecraft/death-rewind',
                'guides/minecraft/just-enough-accidents',
              ],
            },
            'guides/minecraft/quick-start',
            'guides/minecraft/selected-region-backup',
            'guides/minecraft/hot-backup',
            'guides/minecraft/hot-restore',
            'guides/minecraft/knotlink-mod',
            'guides/minecraft/troubleshooting',
          ],
        },
        {
          type: 'category',
          label: '一代时光机（MineBackup）',
          items: [
            'guides/minebackup-v1/overview',
            'guides/minebackup-v1/platform-support',
            'guides/minebackup-v1/installation',
            {
              type: 'category',
              label: '命令行与服务器（CLI）',
              items: [
                'guides/minebackup-v1/cli/overview',
                'guides/minebackup-v1/cli/quick-start',
                'guides/minebackup-v1/cli/ai-assisted-config',
                'guides/minebackup-v1/cli/profile-manifest',
                'guides/minebackup-v1/cli/backup-restore',
                'guides/minebackup-v1/cli/jobs',
                'guides/minebackup-v1/cli/serve',
                'guides/minebackup-v1/cli/linux-systemd',
                'guides/minebackup-v1/cli/windows-task-scheduler',
                'guides/minebackup-v1/cli/reference',
                'guides/minebackup-v1/cli/troubleshooting',
              ],
            },
            'guides/minebackup-v1/first-config',
            'guides/minebackup-v1/first-backup',
            'guides/minebackup-v1/first-restore',
            'guides/minebackup-v1/backup-modes',
            'guides/minebackup-v1/history-and-restore',
            'guides/minebackup-v1/automation',
            'guides/minebackup-v1/special-mode',
            'guides/minebackup-v1/hot-backup',
            'guides/minebackup-v1/filters',
            'guides/minebackup-v1/knotlink-integration',
            'guides/minebackup-v1/cloud-archive',
            'guides/minebackup-v1/data-and-migration',
            'guides/minebackup-v1/logging-and-diagnostics',
            'guides/minebackup-v1/service-mode',
            'guides/minebackup-v1/troubleshooting',
            'guides/minebackup-v1/faq',
          ],
        },
      ],
    },
    {
      type: 'category',
      label: '插件生态',
      items: [
        'plugins/overview',
        'plugins/using-plugins',
        'plugins/knotlink',
        'plugins/knotlink-commands',
        {
          type: 'category',
          label: '插件开发',
          items: [
            'plugins/developing/quick-start',
            'plugins/developing/tutorial',
            'plugins/developing/plugin-api',
            'plugins/developing/hotkey-api',
            'plugins/developing/knotlink-api',
            'plugins/developing/settings-schema',
            'plugins/developing/packaging',
            'plugins/developing/auto-update',
          ],
        },
      ],
    },
    {
      type: 'category',
      label: '项目架构',
      collapsed: true,
      items: [
        'architecture/index',
        'architecture/directory-structure',
        'architecture/patterns',
        'architecture/namespaces',
        'architecture/services',
        'architecture/plugin-system',
        'architecture/data-models',
        'architecture/views',
      ],
    },
    'faq',
  ],
};

export default sidebars;
