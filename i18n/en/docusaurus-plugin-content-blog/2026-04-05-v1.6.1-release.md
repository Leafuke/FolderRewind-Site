...
authors: [leafuke]
tags: [release]
description: FolderRewind 1.6.1 stability update — fixes filter rule matching and restore whitelist issues, splits metadata storage to optimize long-term performance, and enhances automatic task reliability with template interface improvements
---
...
If you use the sideloaded build, run one backup + restore validation in a test directory after upgrading before switching to production data.
:::
...
If you run high-frequency backup tasks, this update should improve stability.
